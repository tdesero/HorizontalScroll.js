  ...
  _createClass(HorizontalScroll, [{
    ...
    value: function setSizes() {
      ...
      for (var i = 0; i < this.wrappers.length; i++) {
        ...
        for (var j = 0; j < this.items[i].length; j++) {
          this.wrapperWidths[i] += this.items[i][j].offsetWidth; //last items height
        ...
        if (this.options.setHeight) {
          this.outerHeights[i] = (this.wrapperWidths[i] - this.windowWidth) / Math.abs(this.options.speed) + this.lastItemHeights[i] + this.options.offsetTop;
        } else {
    ...
    value: function handleScroll() {
      ...
      for (var i = 0; i < this.wrappers.length; i++) {
        ...
        var shouldStick = top < this.options.offsetTop && -1 * top < this.outerHeights[i] - this.lastItemHeights[i] - this.options.offsetTop; //transform value either not limited or with limits. see this.options.moveInOut
